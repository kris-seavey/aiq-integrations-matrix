...
// =============================================================
// PREVIEW A — Smart checkboxes
// One checkbox = view that integration's details.
// Two checkboxes = automatically switch to compare mode.
// The integration name and the checkbox are unified into one click target.
// =============================================================
...
function CheckboxIndicator({
  ...
  // selectedOnDark = the row is currently the active integration card
  // (white-on-purple). We need a checkbox that reads on the purple background.
  let cls = "flex h-5 w-5 shrink-0 items-center justify-center rounded border ";
  if (disabled) {
    cls += "border-[#D7DCE5] bg-[#F4F6FA]";
  } else if (selectedOnDark) {
    cls += "border-white bg-white";
  } else if (checked) {
  ...
  return (
    <span className={cls} aria-hidden="true">
      {checked && (
        <svg
          viewBox="0 0 16 16"
          className={`h-3.5 w-3.5 ${selectedOnDark ? "text-[#6262F5]" : "text-white"}`}
          fill="none"
          stroke="currentColor"
          strokeWidth="3"
          strokeLinecap="round"
...
export default function PublicIntegrationsPagePreviewA() {
  ...
  // Derived: when exactly one is selected, that's the integration on display
  const selectedIntegration = useMemo<Integration | null>(() => {
    if (selectedIds.length !== 1) return null;
    return (
      integrations.find((i) => i.integration_id === selectedIds[0]) ?? null
    );
  }, [selectedIds, integrations]);

  const isComparisonMode = selectedIds.length === 2;

  const compareIntegrations = useMemo<Integration[]>(() => {
    return selectedIds
  ...
  // Load support rows when one integration is selected.
  useEffect(() => {
    if (selectedIntegration && allFeatures.length > 0) {
      loadIntegrationSupport(selectedIntegration);
    } else {
      setRows([]);
      setViewAllFeatures(false);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedIntegration, allFeatures, allSections]);

  // Load comparison when two are selected.
  useEffect(() => {
    if (selectedIds.length === 2 && allFeatures.length > 0) {
      loadComparison(selectedIds);
    } else {
      setComparisonRows([]);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedIds, allFeatures, allSections]);
  ...
  async function loadIntegrationSupport(integration: Integration) {
  ...
  const supportedCount = useMemo(
    () => rows.filter((row) => row.isSupported).length,
    [rows]
  );
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-[12px] font-bold uppercase tracking-[0.12em] text-[#6262F5]">
                  Preview A · Smart checkboxes
                </p>
              </div>
            </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[280px_minmax(0,1fr)]">
          <aside className="h-fit rounded-2xl border border-white/15 bg-white p-5 text-[#080808] shadow-sm">
            ...
            <p className="mt-2 text-[12px] leading-5 text-[#626875]">
              Check one to view details. Check two to compare them.
            </p>
            ...
            <div className="mt-3 space-y-2">
              {filteredIntegrations.map((integration) => {
                ...
                const isSingleView =
                  isChecked && selectedIds.length === 1;

                return (
                  <button
                    key={integration.integration_id}
                    type="button"
                    onClick={() => toggleSelected(integration.integration_id)}
                    disabled={isDisabled}
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div
                          className={`mt-1 text-[13px] ${
                            isSingleView ? "text-white/90" : "text-[#626875]"
                          }`}
                        >
              ...
              <>
                <div className="border-b border-[#E2E6ED] pb-6">
                  <div className="flex flex-col gap-4">
                    ...
                    <p className="text-[14px] leading-6 text-[#626875]">
                      {status}
                    </p>
                    ...
                    <div className="flex flex-col gap-4 pt-2 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <label className="inline-flex items-center gap-3 text-[14px] font-medium text-[#626875]">
                        ...
                        <button
                          ...
                          onClick={() => setViewAllFeatures((p) => !p)}
              ...
              <>
                <div className="border-b border-[#E2E6ED] pb-6">
                  ...
                  <p className="mt-3 max-w-3xl text-[16px] leading-6 text-[#626875]">
                    ...
                    the matrix. Click a feature to see its description, check
                    one integration in the sidebar to view supported features,
                    or check two to compare them.
                  </p>
                </div>
