...
// =============================================================
// PREVIEW B — Explicit "Compare Integrations" mode
// Default UX is clean: no checkboxes, click an integration to view.
// Pressing the "Compare Integrations" button enters compare mode,
// which reveals checkboxes. Pick two and the comparison view appears.
// "Exit compare mode" returns to the default click-to-view state.
// =============================================================
...
export default function PublicIntegrationsPagePreviewB() {
  ...
  const supportedCount = useMemo(
    () => rows.filter((r) => r.isSupported).length,
    [rows]
  );
        ...
        <div className="grid gap-8 lg:grid-cols-[280px_minmax(0,1fr)]">
          <aside className="h-fit rounded-2xl border border-white/15 bg-white p-5 text-[#080808] shadow-sm">
            ...
            <p className="mt-2 text-[12px] leading-5 text-[#626875]">
              {compareMode
                ? `Check exactly two integrations to compare them. ${
                    compareIds.length
                  }/2 selected.`
